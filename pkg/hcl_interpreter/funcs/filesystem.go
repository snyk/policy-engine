...
func MakeFileFunc(fsys afero.Fs, baseDir string, encBase64 bool) function.Function {
	return function.New(&function.Spec{
		...
		Impl: func(args []cty.Value, retType cty.Type) (cty.Value, error) {
			...
			src, err := readFileBytes(fsys, baseDir, path, pathMarks)
			if err != nil {
...
func MakeTemplateFileFunc(fsys afero.Fs, baseDir string, funcsCb func() map[string]function.Function) function.Function {
	...
	loadTmpl := func(fn string, marks cty.ValueMarks) (hcl.Expression, error) {
		...
		tmplVal, err := File(fsys, baseDir, cty.StringVal(fn).WithMarks(marks))
		if err != nil {
			return nil, err
		}
	...
	return function.New(&function.Spec{
		...
		Impl: func(args []cty.Value, retType cty.Type) (cty.Value, error) {
			...
			case (fileType & os.ModeDir) != 0:
				err = function.NewArgErrorf(1, "%s is a directory, not a file", path)
			case (fileType & os.ModeDevice) != 0:
				err = function.NewArgErrorf(1, "%s is a device node, not a regular file", path)
			case (fileType & os.ModeNamedPipe) != 0:
				err = function.NewArgErrorf(1, "%s is a named pipe, not a regular file", path)
			case (fileType & os.ModeSocket) != 0:
				err = function.NewArgErrorf(1, "%s is a unix domain socket, not a regular file", path)
			default:
				// If it's not a type we recognize then we'll just return a
				// generic error message. This should be very rare.
				err = function.NewArgErrorf(1, "%s is not a regular file", path)
...
func MakeFileSetFunc(fsys afero.Fs, baseDir string) function.Function {
	return function.New(&function.Spec{
		...
		Impl: func(args []cty.Value, retType cty.Type) (cty.Value, error) {
			...
			matches, err := doublestar.Glob(afero.IOFS{Fs: fsys}, pattern)
			if err != nil {
			...
			for _, match := range matches {
				fi, err := fsys.Stat(match)

				if err != nil {
					return cty.UnknownVal(cty.Set(cty.String)), fmt.Errorf("failed to stat %s: %w", match, err)
				}
				...
				if err != nil {
					return cty.UnknownVal(cty.Set(cty.String)), fmt.Errorf("failed to trim path of match %s: %w", match, err)
				}
	...
	return function.New(&function.Spec{
		...
		Impl: func(args []cty.Value, retType cty.Type) (cty.Value, error) {
			...
			if _, ok := fsys.(afero.OsFs); ok {
				...
				return cty.StringVal(filepath.ToSlash(absPath)), nil
			}
		},
	})
}
...
func openFile(fsys afero.Fs, baseDir, path string) (afero.File, error) {
	path, err := homedir.Expand(path)
	if err != nil {
...
func readFileBytes(fsys afero.Fs, baseDir, path string, marks cty.ValueMarks) ([]byte, error) {
...
func File(fsys afero.Fs, baseDir string, path cty.Value) (cty.Value, error) {
	fn := MakeFileFunc(fsys, baseDir, false)
...
func FileExists(fsys afero.Fs, baseDir string, path cty.Value) (cty.Value, error) {
	fn := MakeFileExistsFunc(fsys, baseDir)
	return fn.Call([]cty.Value{path})
}
...
func FileSet(fsys afero.Fs, baseDir string, path, pattern cty.Value) (cty.Value, error) {
	fn := MakeFileSetFunc(fsys, baseDir)
...
func FileBase64(fsys afero.Fs, baseDir string, path cty.Value) (cty.Value, error) {
	fn := MakeFileFunc(fsys, baseDir, true)
	return fn.Call([]cty.Value{path})
}
